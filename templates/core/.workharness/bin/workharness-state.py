...
try:
    import fcntl
except ImportError:  # pragma: no cover - workharness hooks currently target Unix-like shells.
    fcntl = None
...
STAGE_SKILLS = {
    "clarify": "workharness-clarify",
    "context_gather": "workharness-context-gather",
    "plan": "workharness-plan",
    "generate": "workharness-generate",
    "evaluate": "workharness-evaluate",
}
ARTIFACTS = {
...
def state_lock(root: Path):
    if fcntl is None:
        raise RuntimeError("workharness state locking requires fcntl")
    lock_dir = harness_dir(root) / "state"
    lock_dir.mkdir(parents=True, exist_ok=True)
    lock_path = lock_dir / "workharness.lock"
...
def initial_run(
    ...
    return {
        ...
        "request": request,
        "mode": mode,
        "activation_source": "workharness_skill" if mode == "auto" else "manual_skill",
        "status": "active",
        "current_stage": stage,
        "workflow": STAGES,
...
def build_commit_prompt(root: Path, state: dict[str, Any], key: str, mode: str, implementation_phase: str | None) -> str:
    ...
    return (
        "# WorkHarness Commit Prompt\n\n"
        "Use the `workharness-commit` skill now. The state runner is only asking for a commit; it has not staged files and has not run `git commit`.\n\n"
        ...
        "```bash\n"
        f"python3 .workharness/bin/workharness-state.py set-commit {key} committed --run-id {state['run_id']}\n"
        f"python3 .workharness/bin/workharness-state.py set-commit {key} no_changes --run-id {state['run_id']} --message \"no eligible changes to commit\"\n"
        f"python3 .workharness/bin/workharness-state.py set-commit {key} skipped --run-id {state['run_id']} --message \"<unsafe or ambiguous commit scope>\"\n"
        f"python3 .workharness/bin/workharness-state.py set-commit {key} failed --run-id {state['run_id']} --message \"<failure summary>\"\n"
        "```\n\n"
...
def build_stage_prompt(root: Path, state: dict[str, Any], stage: str, reprompt: bool = False) -> str:
    ...
    artifact_rel = str(Path(".workharness") / "runs" / run_id / state["stages"][stage]["artifact"])
    ...
    return (
        ...
        .replace("{{RUN_PATH}}", str(Path(".workharness") / "runs" / run_id / "run.json"))
...
def compute_next(
    ...
    if require_auto and (active.get("mode") != "auto" or active.get("activation_source") != "workharness_skill"):
        return result_none("active run is not an auto workharness run")
    ...
    if require_auto and (state.get("mode") != "auto" or state.get("activation_source") != "workharness_skill"):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="WorkHarness file-state runner.")
    ...
    start_new.add_argument("--park-message", default="manual pause before starting a new workharness run")
    ...
    park.add_argument("--message", default="manual pause before starting a new workharness run")
