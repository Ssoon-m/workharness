...
INSTALL_PATH = Path(".workharness") / "install.json"
HOOK_MARKER = ".workharness"
...
def normalize_install(data: dict[str, Any]) -> dict[str, Any]:
    ...
    sync.setdefault("source", ".workharness/skills")
    return install
...
def ensure_state_files(root: Path) -> list[Path]:
    changed: list[Path] = []
    state_dir = root / ".workharness" / "state"
...
def command_for(runtime: str, event: str) -> str:
    ...
    return (
        ...
        "while :; do "
        f"f=\"$dir/.workharness/hooks/{script}\"; "
        "[ -x \"$f\" ] && exec \"$f\"; "
        "[ -e \"$dir/.git\" ] && break; "
        "parent=\"$(dirname \"$dir\")\"; "
        "[ \"$parent\" = \"$dir\" ] && break; "
...
def hook_entry(runtime: str, event: str) -> dict[str, Any]:
    ...
    if runtime == "codex" and event == "stop":
        entry["statusMessage"] = "Checking workharness state"
    return entry


def command_is_workharness(value: Any) -> bool:
...
def merge_hook(data: dict[str, Any], event: str, matcher: str, entry: dict[str, Any]) -> None:
    ...
    for group in groups:
        ...
        if isinstance(existing, list):
            existing[:] = [item for item in existing if not command_is_workharness(item)]
...
def remove_workharness_hooks_except(data: dict[str, Any], keep_events: set[str]) -> None:
    ...
    for event, groups in list(hooks_root.items()):
        ...
        for group in groups:
            ...
            existing = group.get("hooks")
            if isinstance(existing, list):
                group["hooks"] = [item for item in existing if not command_is_workharness(item)]
            if group.get("hooks"):
                next_groups.append(group)
        if next_groups:
...
def install_hooks(root: Path, provider: str) -> list[Path]:
    changed: list[Path] = []
    make_executable(root / ".workharness" / "hooks" / f"{provider}-stop.sh")
    if provider == "codex":
        config_path = root / ".codex" / "config.toml"
        hooks_path = root / ".codex" / "hooks.json"
        ensure_codex_feature_flag(config_path)
        data = load_json_object(hooks_path)
        remove_workharness_hooks_except(data, {"Stop"})
...
def reconcile_provider(root: Path, install: dict[str, Any], provider: str, *, install_hooks_enabled: bool) -> dict[str, Any]:
    ...
    if not isinstance(agent_config, dict):
        agent_config = {}
    source_rel = str(install.get("skill_sync", {}).get("source") or ".workharness/skills")
...
def command_doctor(args: argparse.Namespace) -> int:
    ...
    if not (root / INSTALL_PATH).is_file():
        issues.append({"level": "error", "message": "missing .workharness/install.json"})
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage WorkHarness provider bridges.")
