...
def relative_to_git_root(path: Path, git_root: Path) -> Path:
    try:
        ...
        raise RuntimeError(f"workharness root is not inside git root: {path}") from exc
...
def require_git_repo(root: Path) -> None:
    ...
    if result.returncode != 0 or result.stdout.strip() != "true":
        raise RuntimeError("workharness worktrees require a git worktree")
    head = git(root, "rev-parse", "--verify", "HEAD")
    if head.returncode != 0:
        raise RuntimeError("workharness worktrees require an initial commit")
...
def next_name(root: Path, request: str) -> str:
    ...
    candidate = base
    suffix = 2
    while branch_exists(root, f"workharness/{candidate}") or default_worktree_path(root, candidate).exists():
...
def start_run_in_worktree(
    ...
    runner = harness_root / ".workharness" / "bin" / "workharness-state.py"
    if not runner.exists():
        raise RuntimeError(f"new worktree is missing workharness runner: {runner}")
...
def command_create(args: argparse.Namespace) -> int:
    ...
    branch = args.branch or f"workharness/{run_id}"
    ...
    resume_command = "python3 .workharness/bin/workharness-state.py resume --json"
    next_command = "python3 .workharness/bin/workharness-state.py next --require-auto --reprompt-running --require-session-binding --json"
    output: dict[str, Any] = {
        "run_id": run_id,
        "branch": branch,
        ...
        "handoff": {
            ...
            "note": "Tell the user to open a new Codex/Claude session at the worktree path and ask it to continue through the workharness skill. Do not ask the user to run state scripts directly.",
        },
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create workharness git worktrees for parallel runs.")
    sub = parser.add_subparsers(dest="command", required=True)

    create = sub.add_parser("create", help="create a workharness worktree and branch")
