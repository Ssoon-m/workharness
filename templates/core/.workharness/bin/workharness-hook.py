...
HOOK_TIMEOUT_ENV = "WORKHARNESS_HOOK_TIMEOUT_SECONDS"
...
def state_summary(root: Path) -> dict[str, Any]:
    ...
    if not active_run:
        return summary
    run_path = root / ".workharness" / "runs" / str(active_run) / "run.json"
...
def write_log(
    ...
    try:
        log_dir = root / ".workharness" / "state" / "logs"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Provider Stop hook wrapper for workharness.")
    ...
    try:
        ...
        if not session_id:
            ...
            return no_op(args.runtime)
        runner = root / ".workharness" / "bin" / "workharness-state.py"
        try:
            ...
            log_result = {
                "action": "none",
                "error": (
                    f"workharness next timed out after {timeout_seconds:g}s: "
            ...
            return no_op(args.runtime, f"workharness hook error: {log_result['error']}")
        if result.returncode != 0:
            output = "\n".join(part for part in (result.stdout.strip(), result.stderr.strip()) if part).strip()
            log_result = {"action": "none", "error": output or f"workharness next failed: {result.returncode}"}
            write_log(root, args.runtime, session_id, input_data, log_result)
            return no_op(args.runtime, f"workharness hook error: {log_result['error']}")
        try:
            payload = json.loads(result.stdout or "{}")
        except json.JSONDecodeError as exc:
            log_result = {"action": "none", "error": f"invalid workharness next output: {exc}"}
            write_log(root, args.runtime, session_id, input_data, log_result)
            return no_op(args.runtime, f"workharness hook error: {log_result['error']}")
        if not isinstance(payload, dict):
            payload = {"action": "none", "error": "invalid workharness next payload"}
        ...
        return no_op(args.runtime, f"workharness hook error: {exc}")
