# WorkHarness Continuation

Continue the active workharness run from durable files only.
...
- Update state with `.workharness/bin/workharness-state.py` before ending. A completed stage must be recorded as `completed`; otherwise the Stop hook will re-enter the same stage.
...
If this prompt is a commit prompt rather than a stage prompt, use the `workharness-commit` skill. The state runner and Stop hook never run `git commit`.
