---
name: workharness-context-gather
description: Use when the user explicitly invokes workharness-context-gather, or when a workharness continuation asks for the context-gather stage. Collects repository facts, relevant files, patterns, constraints, risks, docs, and validation commands without product code edits.
---
...
- If `workharness-context-gather` is run directly without a run id, create a manual run:

```bash
python3 .workharness/bin/workharness-state.py start --mode manual --stage context-gather --request "<request>" --commit-mode none --json
```
...
```bash
python3 .workharness/skills/workharness-context-gather/scripts/render-context-config.py
```
...
Write `.workharness/runs/<run-id>/artifacts/context.md`:
...
```bash
python3 .workharness/bin/workharness-state.py set-stage context-gather completed --run-id <run-id>
```
