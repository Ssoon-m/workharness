---
name: workharness
description: Use only when the user explicitly invokes workharness or asks to start/resume the workharness workflow. Runs clarify -> context-gather -> plan -> generate -> evaluate from durable .workharness run files.
---

# WorkHarness

WorkHarness runs a large repository task from durable file state under `.workharness/runs/<run-id>/`.
...
```bash
python3 .workharness/bin/workharness-state.py status --json
```
...
- `start-new-in-worktree`: keep the existing run active here, then create a new git worktree and branch for a separate workharness run.

Use this Korean prompt:

```text
이 worktree에 active workharness run이 있습니다.
...
```bash
python3 .workharness/bin/workharness-state.py resume --json
```
...
```bash
python3 .workharness/bin/workharness-state.py start-new --request "<request>" --loop-count <count> --commit-mode <none|phase|final> --json
```

This command validates the new run, parks the existing active run with `manual_pause`, clears this worktree's active slot, and creates the new active run. It only updates `.workharness` run state. It does not clean the working tree, reset files, or clear git staging. If file isolation is needed, use `start-new-in-worktree` instead.
...
```bash
python3 .workharness/bin/workharness-worktree.py create --request "<request>" --loop-count <count> --commit-mode <none|phase|final> --json
```

This creates the git worktree and also creates an unbound auto run under the new worktree's `.workharness/runs/<run-id>/`. It must not change the current worktree's active run.

Tell the user the worktree path, harness path, branch, and run id. Tell them to open a new Codex/Claude session at the returned worktree path and ask that session to continue through the workharness skill. Use this Korean guidance:

```text
새 Codex/Claude 세션을 worktree path에서 열고, workharness skill로 이 worktree의 active run을 이어서 작업해 달라고 요청하세요.
```

Do not tell the user to run `workharness-state.py` commands directly for the new worktree handoff. Do not run handoff continuation commands in the current session. The current session remains bound to the original worktree. Do not bind a second run to the current session and do not ask the user to repeat the original request.
...
```bash
python3 .workharness/bin/workharness-state.py start --mode auto --request "<request>" --loop-count <count> --commit-mode <none|phase|final> --json
```

6. Get and execute the first continuation prompt:

```bash
python3 .workharness/bin/workharness-state.py next --require-auto --reprompt-running --require-session-binding --json
```
...
- Only runs created by `workharness` with `mode: auto` are eligible for Stop-hook continuation.
...
- One worktree has at most one active workharness run.
- Parallel workharness work must use `workharness-worktree.py create`, which creates:
  - run/worktree name: `YYYYMMDD-HHMMSS-<task-slug>`
  - branch: `workharness/<name>`
  - path: `<repo-parent>/<repo-name>.worktrees/<name>`
  - an unbound auto run under the new worktree's `.workharness/runs/<run-id>/`
- Start a new session whose cwd is the returned worktree path, then ask that session to continue the active run through the workharness skill.

## Manual Skills

- `workharness-clarify`, `workharness-context-gather`, `workharness-plan`, `workharness-generate`, and `workharness-evaluate` can be run directly.
- Direct skill runs perform one stage and stop.
- Manual runs do not activate Stop-hook continuation.
- If the user later says "modify based on that", treat it as a normal direct implementation request unless they explicitly invoke `workharness-generate` with a phase file.
...
- The main session writes artifacts, updates state, and handles commit prompts with `workharness-commit`.
