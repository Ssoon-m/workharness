---
name: workharness-plan
description: Use when the user explicitly invokes workharness-plan, or when a workharness continuation asks for the plan stage. Creates a planning artifact and self-contained phase files without implementation.
---
...
- If `workharness-plan` is run directly without a run id, create a manual run:

```bash
python3 .workharness/bin/workharness-state.py start --mode manual --stage plan --request "<request>" --commit-mode none --json
```
...
Read available artifacts from `.workharness/runs/<run-id>/artifacts/`, especially:
...
Write `.workharness/runs/<run-id>/artifacts/plan.md` with the phase breakdown, rationale, dependency order, and validation strategy.
...
Create one or more phase files under `.workharness/runs/<run-id>/phases/` using `phase-001.md`, `phase-002.md`, and so on.
...
```bash
python3 .workharness/bin/workharness-state.py set-stage plan completed --run-id <run-id>
```
